...
from sms import mensagem_sms as sms
...
@app.route("/feedback_positivo", methods=['POST'])
def enviar_email():
    # Autenticação.
    logado = autenticar_login()
    if logado is None:
        return redirect("/")
    corpo_email = """
    <p>Gostei da experiencia do site, obrigado</p>
    """
    
    msg = email.message.Message()
    msg['Subject'] = "Feedback Positivo"
    msg['From'] = 'stunderbr@gmail.com'
    msg['To'] = 'stunderbr@gmail.com'
    password = 'ejiaeadvdiyzqkre'
    ...
    print('Email enviado')
    return render_template("feedback_positivo.html", logado = logado)

@app.route("/feedback_negativo", methods=['POST'])
def enviar_email2():
    # Autenticação.
    logado = autenticar_login()
    if logado is None:
        return redirect("/")
    corpo_email = """
    <p>Não gostei da experiencia do site.</p>
    """
    
    msg = email.message.Message()
    msg['Subject'] = "Feedback Negativo"
    msg['From'] = 'stunderbr@gmail.com'
    msg['To'] = 'stunderbr@gmail.com'
    password = 'ejiaeadvdiyzqkre'
    msg.add_header('Content-Type', 'text/html')
    ...
    s.sendmail(msg['From'], [msg['To']], msg.as_string().encode('utf-8'))
    print('Email enviado')
    return render_template("feedback_negativo.html", logado = logado)
...
def contato():
    ...
    # Monta a resposta.
    return render_template("feedback.html", logado = logado)
...
def criar_produto_api():
    ...
    # Envio de promocao Demo
    sms(f'Promoção!!!! {nome_produto} está apenas por R${valor}. Corre, restam apenas {quantidade} unidades!!!')
...
REPLACE INTO usuario (login, senha, nome) VALUES ('admin', '12345', 'Admin');
"""
